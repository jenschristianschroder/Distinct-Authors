...
function linkedPostId(row) {
  return String(row?.link_id || row?.post_id || '').replace(/^t3_/i, '').trim();
}

function itemFromRow(row, kind, postTitles = null) {
  const raw = landscape.clean(landscape.rowText(row, kind), kind === 'post' ? 700 : 560);
  if (!raw) return null;
  let text = `${kind.toUpperCase()} | ${raw}`;
  if (kind === 'comment' && postTitles instanceof Map) {
    const title = landscape.clean(postTitles.get(linkedPostId(row)) || '', 220);
    if (title) text = `COMMENT ON POST: ${title} | COMMENT: ${raw}`;
  }
  return {
...
function mixedSample(rows, kind, target, postTitles = null) {
  const items = (rows || []).map(row => itemFromRow(row, kind, postTitles)).filter(Boolean);
...
function sampleCorpus(posts, comments, limit = 240) {
  ...
  const postTitles = new Map((posts || []).map(post => [String(post?.id || ''), String(post?.title || '')]).filter(([id]) => id));
  ...
  let result = [...mixedSample(posts, 'post', postTarget), ...mixedSample(comments, 'comment', commentTarget, postTitles)];
  if (result.length < total) {
    const ids = new Set(result.map(item => `${item.kind}:${item.id}`));
    for (const item of [...mixedSample(posts, 'post', total), ...mixedSample(comments, 'comment', total, postTitles)]) {
...
function clusterEvidence(items, vectorsInput, requestedK, options = {}) {
  ...
  return clusters.filter(cluster => cluster.members.length).map(cluster => {
    ...
    const representative = ranked.slice(0, 8).map(entry => entry.item);
    ...
    const phrases = landscape.candidatePhrases(clusterPosts, clusterComments, 14, options);
    return {
      id: cluster.id,
