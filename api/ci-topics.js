...
async function verifyGitHubActionsToken(token) {
  ...
  const { payload } = await jwtVerify(token, remoteJwks, { issuer: ISSUER, audience: AUDIENCE, clockTolerance: 10 });
  ...
  if (!['push', 'workflow_dispatch'].includes(String(payload.event_name || ''))) throw new Error('Unexpected GitHub Actions event.');
  return payload;
}

module.exports = async function ciTopics(req, res) {
  res.setHeader('Cache-Control', 'no-store');
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed.' });
  try { await verifyGitHubActionsToken(bearerToken(req)); }
  catch (error) { return res.status(401).json({ error: `GitHub Actions OIDC verification failed: ${error.message}` }); }
  ...
  const fakeReq = {
    ...
    body: { subreddit: 'TheTowerGame', start: '2026-09-01', end: '2026-09-05', topics: 6 }
  };
  return topics(fakeReq, res);
};
